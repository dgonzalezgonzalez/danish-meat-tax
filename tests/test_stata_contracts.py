...
class StataContractsTest(unittest.TestCase):
    ...
    def test_analysis_windows_and_estimators_are_stata_owned(self):
        ...
        self.assertIn("gen int event_time = relative_time", micro)
        self.assertNotIn("replace event_time = relative_time - 1 if relative_time > 0", micro)
        self.assertNotIn("local adjustment_terms", aggregate)
