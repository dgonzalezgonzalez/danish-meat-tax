...
forvalues lag = 1/12 {
    gen double lag_cpi_`lag' = L`lag'.ln_cpi
}
...
local sc_covariates lag_cpi_1 lag_cpi_2 lag_cpi_3 lag_cpi_4 lag_cpi_5 lag_cpi_6 lag_cpi_7 lag_cpi_8 lag_cpi_9 lag_cpi_10 lag_cpi_11 lag_cpi_12 `category_covariates'
...
quietly summarize unit if beef, meanonly
local treated_unit = r(min)
quietly count
...
* -------------------------------------------------------------------------
* Synthetic control: same synth command and predictors as the OECD analysis,
* now on a symmetric 15-month pre/post window with a corrected donor pool.
* -------------------------------------------------------------------------
synth ln_cpi `sc_covariates', trunit(`treated_unit') trperiod(`event') ///
    resultsperiod(`window_start'(1)`window_end') keep("outputs/models/stata/aggregate_sc_path.dta") replace

preserve
use "outputs/models/stata/aggregate_sc_path.dta", clear
rename _time month
format month %tm
gen double gap = _Y_treated - _Y_synthetic
quietly summarize gap if month >= `event', meanonly
local sc_att = r(mean)
quietly summarize gap if month < `event', meanonly
local sc_pre_mean = r(mean)
gen double sq_gap = gap^2
quietly summarize sq_gap if month < `event', meanonly
local sc_pre_rmspe = sqrt(r(mean))
quietly summarize sq_gap if month >= `event', meanonly
local sc_post_rmspe = sqrt(r(mean))
local sc_ratio = `sc_post_rmspe' / `sc_pre_rmspe'
keep month _Y_treated _Y_synthetic gap
rename _Y_treated treated
rename _Y_synthetic synthetic
export delimited using "outputs/models/stata/aggregate_sc_series.csv", replace
twoway ///
    (line treated month, lcolor(black) lwidth(medthick)) ///
    (line synthetic month, lcolor(gs7) lpattern(dash) lwidth(medthick)), ///
    legend(order(1 "Beef and veal" 2 "Synthetic control") rows(1) position(6)) ///
    xtitle("") ytitle("Log consumer price index") graphregion(color(white)) plotregion(color(white))
graph export "outputs/figures/stata/aggregate_sc.png", width(2200) replace
restore

tempfile placebo_results
tempname placebo_post
postfile `placebo_post' int unit double rmspe_ratio using `placebo_results', replace
foreach placebo_unit of local all_units {
    capture quietly synth ln_cpi `sc_covariates', trunit(`placebo_unit') trperiod(`event') ///
        resultsperiod(`window_start'(1)`window_end') keep("outputs/models/stata/sc_placebo_`placebo_unit'.dta") replace
    if !_rc {
        preserve
        quietly use "outputs/models/stata/sc_placebo_`placebo_unit'.dta", clear
        quietly gen double placebo_gap = _Y_treated - _Y_synthetic
        quietly gen double placebo_sq_gap = placebo_gap^2
        quietly summarize placebo_sq_gap if _time < `event', meanonly
        local placebo_pre = sqrt(r(mean))
        quietly summarize placebo_sq_gap if _time >= `event', meanonly
        local placebo_post_rmspe = sqrt(r(mean))
        if `placebo_pre' > 0 & !missing(`placebo_pre', `placebo_post_rmspe') {
            post `placebo_post' (`placebo_unit') (`placebo_post_rmspe' / `placebo_pre')
        }
        restore
        erase "outputs/models/stata/sc_placebo_`placebo_unit'.dta"
    }
}
postclose `placebo_post'
preserve
use `placebo_results', clear
count if rmspe_ratio >= `sc_ratio'
local sc_p = r(N) / _N
export delimited using "outputs/models/stata/aggregate_sc_placebos.csv", replace
restore
...
post `estimates_post' ("aggregate_did") (`did_att') (`did_se') (`did_p') (`did_low') (`did_high') ///
    (`did_n') (`panel_units') (30) (`pre_beef_cpi') (`did_r2') ("2023m4-2025m9") ("Yes") ("No") ("HAC Newey-West lag 2")
post `estimates_post' ("aggregate_sc") (`sc_att') (.) (`sc_p') (.) (.) ///
    (`panel_observations') (`panel_units') (30) (`pre_beef_cpi') (.) ("2023m4-2025m9") ("No") ("Yes") ("space placebo")
