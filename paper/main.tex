...
The dashed surfaces combine uncertainty in the preferred price coefficient with the hierarchical SCC variation. For each of the 10,000 SCC draws, I independently draw a price coefficient centered on the preferred estimate, using its HAC standard error and a Student $t$ distribution with 28 degrees of freedom, matching the reference distribution used for Table~\ref{tab:main}. I substitute both draws into equation~\eqref{eq:persistence_gap} and report the 2.5th and 97.5th percentiles of the resulting gap at each parameter combination. Independence between the SCC and coefficient draws is an explicit assumption. The price benchmark, emissions intensity, exchange rate, and policy rate remain fixed. Because the SCC variation represents sensitivity across studies and modeling assumptions, the resulting bounds are pointwise combined sensitivity intervals rather than frequentist confidence bands. At $a=0$, coefficient uncertainty drops out but SCC uncertainty remains.

Even at $a=f=\lambda=1$, the combined 95 percent sensitivity interval for the remaining gap is DKK 11.04--132.35/kg. Its width shows how strongly uncertainty about climate damages affects the comparison. The minimum point gap over the entire parameter cube is DKK 47.61/kg, so none of these combinations closes the gap at the central damage benchmark. Full persistence and full additional pass-through together cover about 28 percent of that benchmark. With no additional implementation response, the announcement alone covers at most about 16 percent at the point estimate. Complete closure would require lower environmental damages or a price response beyond the assumed range: at full persistence and pass-through, the SCC threshold is about USD 44.55/tCO2. A smaller taxable share or weaker persistence lowers that threshold further. These comparisons clarify which assumptions could overturn the conclusion without treating future adjustment as already observed.
