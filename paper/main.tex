...
To examine whether the price result depends on using other foods as controls, I use Eurostat's monthly Harmonised Index of Consumer Prices for beef and veal \citep{eurostathicp2026}. These indices measure consumer-price developments under a common European framework. All 27 EU Member States have positive, non-missing observations throughout April 2023--September 2025, giving 810 country--month observations. Denmark is the exposed country and the other 26 Member States form the donor pool. The Danish HICP is compiled by Statistics Denmark, so this exercise changes the comparison group while retaining closely related underlying consumer-price measurement. It does not provide an independent source check of Danish prices.

A separate check uses the European Commission's Beef Trade Data Explorer, which draws on Eurostat COMEXT data under statistical regime 4. The data report extra-EU beef trade by Member State, external partner, month, and CN8 product. I aggregate imports across beef products to monthly carcass-weight tonnes and balance the panel by coding unreported importer--partner--months as zero. The resulting panel contains 383 importer--partner pairs, including 17 pairs with Denmark as importer, over the same 30-month window.
...
The country-level consumer-price comparison gives an ATT of 0.0157 (placebo standard error 0.0293; $p=0.593$), corresponding to a point increase of about 1.6 percent. Its 95 percent interval, $[-0.0417,0.0731]$, includes both zero and the preferred within-Denmark estimate (Appendix Figure~\ref{fig:b_country_sdid}). The smaller point estimate indicates that the estimated magnitude depends on the comparison group. The country design does not independently establish a price increase, although its uncertainty is also compatible with the preferred estimate. The extra-EU import estimate is $-0.0801$ (bootstrap standard error 0.0782; $p=0.306$; Appendix Figure~\ref{fig:b_trade_sdid}). Both positive and negative import responses remain compatible with its uncertainty. Intra-EU imports are outside this dataset.
